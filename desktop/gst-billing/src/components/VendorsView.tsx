...
import { Truck, Plus, Search, RefreshCw, Edit2, MapPin, Eye } from "lucide-react";
...
export default function VendorsView() {
  ...
  const [form, setForm] = useState({
    name: "", gstin: "", phone: "", address: "", state_code: ""
  });
  ...
  const loadVendors = async () => {
    try {
      setLoading(true);
      const data: Vendor[] = await invoke("get_vendors");
      setVendors(data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const payload = { ...form };
      if (editingId) {
        await invoke("update_vendor", { id: editingId, ...payload });
      } else {
        await invoke("add_vendor", payload);
      }
  ...
  const resetForm = () => {
    setEditingId(null);
    setForm({ name: "", gstin: "", phone: "", address: "", state_code: "" });
  };
  ...
  if (viewingVendorId) {
    return <VendorDetails vendorId={viewingVendorId} onBack={() => setViewingVendorId(null)} />;
  }
    ...
    <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-light tracking-tight flex items-center gap-3">
            ...
            Vendor <span className="font-bold text-white">Directory</span>
          </h2>
          <p className="text-zinc-500 font-medium text-sm mt-1">
            Maintain your supplier directory for Purchase Management and inward GST tracking.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-1 glass p-6 rounded-[2rem] border border-zinc-800/50 flex flex-col items-center justify-center text-center h-48 border-dashed">
            <div className="text-4xl font-black">{vendors.length}</div>
            <div className="text-zinc-500 text-xs font-bold uppercase tracking-widest mt-2">Total Suppliers</div>
        </div>
        ...
        <div className="lg:col-span-3 glass rounded-[2rem] border border-zinc-800/50 overflow-hidden relative">
          <div className="p-4 border-b border-zinc-800/50 flex items-center justify-between bg-zinc-900/20">
            ...
            <button onClick={loadVendors} className="p-2 text-zinc-400 hover:text-white transition-colors">
          ...
          <div className="divide-y divide-zinc-800/50 max-h-[600px] overflow-y-auto">
            {loading && vendors.length === 0 ? (
              <div className="p-12 text-center text-zinc-500">Loading suppliers...</div>
            ) : filtered.length === 0 ? (
              <div className="p-12 text-center">
                <Truck className="w-8 h-8 text-zinc-700 mx-auto mb-3" />
                <p className="text-zinc-400 font-medium">No vendors recorded.</p>
              </div>
            ) : (
              filtered.map(v => (
                <div key={v.id} className="p-6 hover:bg-zinc-900/30 transition-colors flex items-center justify-between group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-white text-base leading-tight flex items-center gap-2">
                        {v.name}
                        {v.gstin && <span className="bg-blue-500/10 text-blue-400 border border-blue-500/20 text-[10px] px-1.5 py-0.5 rounded font-mono uppercase tracking-widest">Linked</span>}
                      </h4>
                      <p className="text-zinc-500 text-xs mt-1 flex items-center gap-3">
                        {v.gstin ? <span className="font-mono">GST: {v.gstin}</span> : <span>Unregistered</span>}
                        {v.state_code && <span className="flex items-center gap-1"><MapPin className="w-3 h-3" /> State {v.state_code}</span>}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => setViewingVendorId(v.id)} className="p-2 bg-zinc-800/50 border border-zinc-700 rounded-lg text-rose-400 hover:text-rose-300 hover:bg-zinc-800 transition-all opacity-0 group-hover:opacity-100" title="View Full Ledger">
                      <Eye className="w-4 h-4" />
                    </button>
                    <button onClick={() => editVendor(v)} className="p-2 bg-zinc-800/50 border border-zinc-700 rounded-lg text-zinc-400 hover:text-white hover:bg-zinc-800 transition-all opacity-0 group-hover:opacity-100" title="Edit Profile">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#09090b] border border-zinc-800 shadow-2xl rounded-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6">
              <h3 className="text-xl font-bold mb-1">{editingId ? 'Edit Vendor Profile' : 'New Vendor Registration'}</h3>
              <p className="text-sm text-zinc-500 mb-6 border-b border-zinc-800/50 pb-4">Provide valid vendor details for Purchase Ledger and Input Tax Credit mapping.</p>
              
              <form onSubmit={handleSave} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2 col-span-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-zinc-500">Business / Supplier Name <span className="text-red-500">*</span></label>
                  ...
                  <div className="space-y-2">
                    ...
                    <input pattern="^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$" value={form.gstin} onChange={e => setForm({...form, gstin: e.target.value.toUpperCase()})} placeholder="Leave blank if unreg" className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-white/20 font-mono text-zinc-300 uppercase" />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-zinc-500">Origin Address</label>
