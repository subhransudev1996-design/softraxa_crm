...
export default function PurchaseBuilder({ onCancel }: { onCancel: () => void }) {
  ...
  const isInterState = selectedVendor?.state_code && settings?.state_code && 
                       (selectedVendor.state_code.slice(0, 2) !== settings.state_code.slice(0, 2));
  ...
  const handleVariationSelect = (index: number, variationId: string) => {
    ...
    newItems[index] = { 
      ...newItems[index], 
      variationId, 
      price: v.purchase_price || 0, // Default to purchase price instead of selling
  ...
  const handleSave = async () => {
    ...
    if (!purchaseNo.trim()) return alert("Enter vendor invoice number");
    ...
    try {
      ...
      // Execute Additions via the robust ERP Engine
      for (const item of validItems) {
        await invoke("execute_stock_addition", {
      ...
      console.error(e);
      alert("Failed to save and post ledger. Error: " + (e instanceof Error ? e.message : JSON.stringify(e)));
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-8 duration-500 pb-20">
      <div className="flex items-center gap-4 border-b border-zinc-800/50 pb-6">
        ...
        <div>
          <h2 className="text-3xl font-light tracking-tight text-white flex gap-2">ERP <span className="font-bold">Purchase Entry</span></h2>
          <p className="text-zinc-500 font-medium text-xs uppercase tracking-widest mt-1">Multi-level Inventory Addition Engine</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="glass p-6 rounded-[2rem] border border-zinc-800/50 flex gap-6 z-10 relative">
            <div className="flex-1">
              <label className="text-xs font-bold uppercase tracking-widest text-zinc-500 block mb-2">Supplier Entity</label>
              <select 
            ...
            <div className="w-48">
              <label className="text-xs font-bold uppercase tracking-widest text-zinc-500 block mb-2">Vendor Invoice No <span className="text-red-500">*</span></label>
              <input value={purchaseNo} onChange={e => setPurchaseNo(e.target.value)} placeholder="ENTER INV NO" className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-emerald-500/50 text-emerald-400 font-mono uppercase placeholder:text-zinc-700 focus:text-white transition-colors" />
            </div>
          </div>

          <div className="glass p-6 rounded-[2rem] border border-zinc-800/50 overflow-hidden relative">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-bold text-lg flex gap-2 items-center"><Layers className="w-5 h-5" /> Receipt Matrix</h3>
              <button 
                    ...
                    <div className="flex gap-4 items-center pl-2 text-sm border-t border-zinc-800/50 pt-3 mt-1">
                      <div className="flex items-center gap-2">
                        ...
                        <input type="number" min="1" value={item.qty} onChange={e => {
                          ...
                          setItems(newItems);
                        }} className="w-20 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-lg px-2 py-1 outline-none text-center font-mono font-bold focus:border-emerald-500 transition-colors" />
                      </div>
                      ...
                      <button onClick={() => setSerialPickerItemIndex(index)} className="flex items-center gap-2 text-xs font-bold px-3 py-1.5 rounded-lg border border-zinc-700 hover:bg-zinc-800 transition-colors shadow-sm text-zinc-300">
                        ...
                        {item.selectedSerials.length > 0 ? <span className="text-emerald-400">{item.selectedSerials.length} Hardware Units Tracked</span> : <span className="text-zinc-400">Scan Inwards Units</span>}
                      </button>
        ...
        <div className="space-y-6">
          <div className="glass p-6 rounded-[2rem] border border-zinc-800/50 relative overflow-hidden h-full flex flex-col">
            ...
            <div className="space-y-4 text-sm font-medium flex-1">
              ...
              {!isInterState ? (
                <>
                  <div className="flex justify-between items-center text-zinc-400"><span>CGST ITC</span><span className="font-mono text-emerald-400">₹{totals.cgst_total.toFixed(2)}</span></div>
                  <div className="flex justify-between items-center text-zinc-400"><span>SGST ITC</span><span className="font-mono text-emerald-400">₹{totals.sgst_total.toFixed(2)}</span></div>
                </>
              ) : (
                <div className="flex justify-between items-center text-zinc-400"><span>IGST ITC Inter-State</span><span className="font-mono text-emerald-400">₹{totals.igst_total.toFixed(2)}</span></div>
              )}
              
              <div className="pt-6 mt-4 border-t border-zinc-800/50">
                <div className="text-xs uppercase tracking-widest font-bold text-zinc-500 mb-1">Total Payload</div>
            ...
            <div className="mt-8 space-y-3">
              <button disabled={loading} onClick={handleSave} className="w-full bg-emerald-500 text-black py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-emerald-400 transition-all shadow-elevated active:scale-95 disabled:opacity-50">
                <Save className="w-4 h-4" /> Commit Stock Addition
              </button>
            </div>
          </div>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#09090b] border border-zinc-800 shadow-2xl rounded-2xl w-full max-w-2xl p-6 flex flex-col max-h-[85vh]">
        <div className="flex justify-between items-center mb-6 shrink-0">
          <div>
            <h3 className="text-xl font-bold">Inward Hardware Log</h3>
            <p className="text-zinc-500 font-mono text-xs">{item.name} {'>'} {item.varName}</p>
          </div>
          <div className="text-right">
            <div className="text-2xl font-bold text-emerald-400">{serials.length} Incoming</div>
            <div className="text-xs uppercase tracking-widest text-zinc-500">Auto-Quantity Sync</div>
          </div>
        </div>
        ...
        <form onSubmit={handleAdd} className="flex gap-4 p-4 glass rounded-2xl border border-zinc-800/50 mb-4 shrink-0 relative overflow-hidden">
            ...
            <button type="submit" className="bg-emerald-500 text-black px-8 py-4 rounded-xl font-bold shadow-elevated hover:bg-emerald-400 transition-colors">Queue Unit</button>
        </form>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 pr-2 pb-4 space-y-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {serials.map((s, idx) => (
                <div key={idx} className="p-4 rounded-xl border border-emerald-500/20 bg-emerald-500/5 flex items-center justify-between group">
                  <div className="font-mono text-sm font-bold text-emerald-300 truncate max-w-[120px]">{s}</div>
        ...
        <div className="flex gap-4 pt-6 border-t border-zinc-800/50 mt-4 shrink-0">
          ...
          <button onClick={() => onConfirm(serials)} className="flex-1 py-4 bg-emerald-500 text-black font-bold rounded-xl shadow-elevated">Confirm Receipt Pool</button>
        </div>
      </div>
    </div>
  );
